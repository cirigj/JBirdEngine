Shader "Custom/IridescentHSVSpaceShadows" {
	Properties {
		...
		_LumaCutoff ("Luma Cutoff", Range(0.01,1)) = .5
	}
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			fixed4 cMain = tex2D (_MainTex, screenUV.xy);
			float2 scrollUV = float2((screenUV.x + _Time.x * _Scroll.x) % 1, (screenUV.y + _Time.x * _Scroll.y) % 1);
			fixed4 cCloud = tex2D (_CloudTex, scrollUV.xy);
			fixed4 cStars = tex2D (_StarsTex, screenUV.xy);

			cStars *= _StarBrightness * max(sin(screenUV.x * 100 + screenUV.y * 50 + _Time.w), 0);
            fixed4 cSpace = cMain + cCloud * cCloud.a;

			half t = getAngleBetweenVectors(IN.viewDir, IN.worldNormal);
			half w = getLerpHue(t) / 360;
			//o.Albedo = half3(t, t, t);

			half3 col = getColor(IN.viewDir, IN.worldNormal);
			float luma = col.r * 0.299 + col.g * 0.587 + col.b * 0.114;
			float mix = 1 - min(max(luma / _LumaCutoff, 0), 1);

			o.Albedo = col * (1 - mix) + cSpace * mix + cStars * mix;
